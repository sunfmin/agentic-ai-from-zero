...
空转是看清楚 fleet 眼里那盘棋的安全办法。等你准备好让它真的动手，你要授权它 —— 而这是全书让你打开的最有后果的一件事，所以慢慢读：

> ⚠️ 真的跑 `/afk-fleet`，意味着：它会 **把分支 push 到你的远端，并把变绿的 PR 自动合并进 `main`，而不再每次问你**。这正是它的用途 —— 无人值守就是无人值守 —— 但你只应该在「自动合进 `main` 是可以接受的」仓库上授权它（一个练手仓库、或一个部署前另有把关的项目）。fleet 的职责 **到「绿灯合进 `main`」为止**；再往后的发布，仍是一个单独的、由人来做的步骤。
...
> 🛠 试一试
>
> 准备好了，就跑 `/afk-fleet`（不带 `--plan`），把它要的授权给它。
>
...
- **afk-fleet**：作者自己的 skill，搭在 orca 上 —— launcher 派生一次性的 tick，给每个 `ready-for-agent` issue 派一个 worker，给每个 PR 把关，把绿的无人值守地自动合并。
- 永远先 `/afk-fleet --plan`；只在「自动合进 `main` 可接受」的地方授权真跑。那道 gate（CI，或者对文字用对抗式验证）才是护住 `main` 的东西。
- 两个工具你都是用，不是造。
