...
你已经读完这本书。你会在终端里导航，会跑 Claude Code，会手写一个 `SKILL.md`，会用渐进式展开把它拆成多文件，也会用 `/skill-creator` 通过对话设计新的 skill；进阶篇里，你还用一套专业 skill 跑通了「立协议 → 逼成决策 → 综合 PRD → 切成 issue」这条规划流水线，并对已有代码做过一次架构深化；执行篇里，你把那份 backlog 跑通了 —— 用 orca 把 agent 铺到并行的 worktree 上，再让一支无人值守的 afk-fleet 把它实现、合并。这本书要教的，到此为止。
