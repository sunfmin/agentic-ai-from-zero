...
3. **`/to-prd`** —— 把所有决策综合成一份 PRD，发成 GitHub issue
4. **`/to-issues`** —— 把 PRD 切成 11 个可独立 grab 的 vertical slice，全部发成 issue
...
后面用的所有 skill（grill、to-prd、to-issues）都会 **读这几个文件**，从而知道：
...
光秃秃一个命令，没有任何额外说明。`to-prd` 的设计就是 **不要面试用户、直接综合已有 context**。
...
按 `to-prd` 模板（problem / solution / user stories / implementation decisions / testing decisions / out of scope / further notes）：
...
之前 setup 阶段只是把 label **约定** 写进了 `docs/agents/triage-labels.md`，GitHub 仓库本身还没创建这些 label。`to-prd` 在创建 issue 时调 `gh label list` 发现少了 4 个（`needs-triage`、`needs-info`、`ready-for-agent`、`ready-for-human`），就直接补创了，再发 issue。
...
如果跳过 grill 直接 `/to-prd`，得到的会是一份"听起来合理但每条都泛泛"的 PRD —— 因为决策从没具体下来过。

---

## 第 5 步：把 PRD 切成 11 个 vertical slice — `/to-issues`
...
又是光秃秃一个命令。`to-issues` 自己知道去读 issue #1。
...
`/to-prd` 和 `/to-issues` 没有再做新设计 —— 它们只是把 grill 已经定下的决策塞进 PRD 模板和 issue 模板。所以这两步快且不容易出错。如果 grill 不够深入，PRD 就会泛泛 —— 但这是 grill 的问题，不是 to-prd 的问题。
...
Issue #1 的 PRD 是这个仓库未来所有变更的"宪法"；新的需求要么走 PRD-prefix issue 单独开（用 `/to-prd`），要么作为 sub-task 挂到 issue #1 之下（用 `/to-issues`）。
